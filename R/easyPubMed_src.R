...
#' for querying PubMed (standard PubMed synthax, see reference for details), or a
#' valid URL for a curated PubMed bibliography.
            ...
            encoding = "UTF8")
  {
    ...
    papers.authors.list <- lapply(1:length(paper.data), (function(i) {
      ...
      if (regexpr("MeshHeadingList", text = art) > 0) {
        out.mesh <- tryCatch({custom_grep(xml_data = art, tag = "MeshHeadingList", format = "character")},
                             error = function(e) NA)
        out.mesh <- tryCatch({custom_grep(xml_data = out.mesh[1], tag = "MeshHeading", format = "list")},
                             error = function(e) NA)
